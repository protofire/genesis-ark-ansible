...
def delete_dir(dir):
    ...
    for file_name in file_names:
        ...
        if os.path.exists(file_path):
            logger.info("remove file", path=file_path)
            os.remove(file_path)
    ...
    for subdir_name in subdir_names:
        ...
        if os.path.exists(subdir_path) and is_empty(subdir_path):
            logger.info("remove subdirectory", path=subdir_path)
            os.removedirs(subdir_path)

    # Remove parent directory
    if os.path.exists(dir_path) and is_empty(dir_path):
        logger.info("remove parent directory", path=dir_path)
...
def rotate_artifacts(max_age_days: int) -> None:
    ...
    logger.info("jobs_dir_path", path=jobs_dir_path)
    ...
    for job_dir in os.walk(jobs_dir_path, topdown=False):
        ...
        if job_dir_path == jobs_dir_path:
            logger.info("skip root directory", path=job_dir_path)
            continue

        # skip current path is it has already been deleted
        if not os.path.exists(job_dir_path):
            logger.warning("path already deleted", path=job_dir_path)
            continue
